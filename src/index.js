...
$(function () {
    ...
    window.activateLayer = function(layerObj) {
        // Remove any existing Colorful/Neutrino base layer from the map
        var map = window.map;
        if (!map) {
            map = window._olMapInstance || (window._olMapInstance = ol.Map ? ol.Map.instances && ol.Map.instances[0] : null);
        }
        if (!map) {
            map = $("#map").data("olMap");
        }
        ...
        if (!map) {
            // fallback: try to find the map instance
            if (window.config && window.config.layers) {
                var testMap = null;
                try {
                    testMap = new ol.Map({layers: window.config.layers});
                } catch (e) {}
                if (testMap) map = testMap;
            }
        }
        if (!map) {
        ...
        // Remove both Colorful and Neutrino layers if present
        map.getLayers().getArray().forEach(function(l) {
            var t = l.get && l.get('title');
            if (t === 'OSM Shortbread Colorful' || t === 'OSM Shortbread Neutrino') {
                map.removeLayer(l);
            }
        });
        // Create and add the selected layer
        var title = layerObj.title || (layerObj._olLayerGroup && layerObj._olLayerGroup.get && layerObj._olLayerGroup.get('title'));
        var newLayer = null;
        if (title === 'OSM Shortbread Colorful') {
            newLayer = new ol.layer.VectorTile({
            ...
            fetch('src/assets/colorful.json')
                ...
                .then(text => {
                    ...
                    olms.applyStyle(newLayer, style, 'versatiles-shortbread');
                });
            ...
            fetch('src/assets/neutrino.json')
                ...
                .then(text => {
                    const style = JSON.parse(text);
                    olms.applyStyle(newLayer, style, 'versatiles-shortbread-neutrino');
                });
        }
        if (newLayer) {
            map.getLayers().insertAt(0, newLayer); // Add as the first base layer
        }
